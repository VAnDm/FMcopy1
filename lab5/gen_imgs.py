...
with open('figures.tex', 'w', encoding='utf-8') as f:
    ...
    for T, V, dt, dv in combos:
        ...
        f.write("    \\centering\n")
        f.write(f"    \\includegraphics[width=0.95\\textwidth]{{images/T{T}_V{V}_dt{dt:.3f}_dv{dv:.3f}_signal.png}}\n")
        f.write(f"    \\caption{{Восстановление сигнала: $T={T}$, $dt={dt}$.}}\n")
        f.write("\\end{figure}\n\n")
        
        f.write("\\begin{figure}[H]\n")
        f.write("    \\centering\n")
        f.write(f"    \\includegraphics[width=0.95\\textwidth]{{images/T{T}_V{V}_dt{dt:.3f}_dv{dv:.3f}_spectrum.png}}\n")
