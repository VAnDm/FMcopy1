...
for i = 1:length(T_arr)
    ...
    H = freqresp(w, omega);
    H = fftshift(squeeze(H));
    y_fur = fftshift(fft(y));
    Y_fur = y_fur .* H;
    y_fil_rev = ifft(ifftshift(Y_fur));
    ...
    legend("Фильтрованный сигнал", "Обратное преобразование",  'Location', 'northeast', fontsize=14);
    set(gcf, 'Units', 'pixels', 'Position', [100, 100, 1200, 800]);
    ...
    figure;
    plot(omega, abs(Y_fur), 'LineWidth', 0.5, color="red");
    hold on;
